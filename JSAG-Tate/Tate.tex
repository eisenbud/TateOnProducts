...
$$
\fix{I changed it to ``Euler polynomial'' to match the name in the package''.}
...
%A polynomial $\sum_m d_mh^m$ in the $i$-th row and $j$-th column indicates that the dimension
...
\]
where $[-1]$ indicates that the sheaf lies in cohomological degree $1$.   This can be considering the Betti table of the strandard:
\begin{verbatim}
betti s
             -1  0 1 2 3 4  5  6
o76 = total: 20 12 6 2 2 6 12 20
          ...
          3:  .  . . . 2 6 12 20
\end{verbatim}
\fix{This is what the code spits out, but it looks off to me.  Isn't the shift too much?  I think the issue comes from the TateResolution code. See my email.}
...
%Corner complexes, as defined in~\cite[\S3]{EES} are the basis of all our computations involving Tate resolutions on $\PP$.  
...
\daniel{The corner complex code is messed up because it only computes based on the Tate resolution, whereas it should work the opposite way around.  See my email.}
%{\small 
%\begin{verbatim}
