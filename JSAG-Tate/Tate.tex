...
B = beilinsonMonad M
\end{verbatim}}
produces a complex $B$ of graded $S$-modules such that the corresponding complex $\widetilde{B}$ of sheaves on $\PP$ is the Beilinson monad $\bU(\sF)$.  For example, if we take $M$ to be a twist of the maximal ideal in the homogeneous
coordinqate ring $S$ of $\PP=\PP^1\times \PP^2$, we can execute:
{\small \begin{verbatim}
M =  S^{{1,1}} ** ker vars S
...
The construction of the Tate resolution of a coherent sheaf $\sF$ on $\PP$ starts with a multigraded $S$-module $M$ representing $\sF$. At a first step we determine a muti-degree $c \in \ZZ^t$ such that $bgg(M_{ \ge c})$ is acyclic.
