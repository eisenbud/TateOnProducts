...
>>>>>>> ea0b9cc43b3b0b0cbe2d12abdea5743ae8a91b7d
...
The far left vertical column of this matrix corresponds to the subquotient of $\bT(\sF)$ consisting of summands generated in degree $(-3,i)$ for some $i$.  That subcomplex, which only involves exterior variables corresponding to the $\PP^2$-factor, is the Tate resolution of $R\pi_*(\sF(-3,0))$ on $\PP^2$.  It can be computed by
\begin{verbatim}
...
Since we can construct a Beilinson monad from a Tate resolution, as described in \S\ref{sec:beilinson monad}, we can compute a Beilinson monad for $R\pi_*(\sF(-3,0))$ directly from this strand of its Tate resolution.
...
\end{verbatim} }
%\noindent returns a complex of graded $T$-modules representing $R\pi_* \sF \in D^b(\PP^m)$.
...
% is a vector bundle on $\P^1$ of rank 2.
...
Corner complexes, as defined in~\cite[\S3]{EES} are the basis of all our computations involving Tate resolutions on $\PP$.  We begin with a finitely generated, multi-graded module $M$ representing a coherent sheaf $\sF$ on $\PP$.  

We first use the function {\tt coarseMultigradedRegularity(M)}  to find a muti-degree $c \in \ZZ^t$ such that $bgg(M_{ \ge c})$ is acyclic.  (This is closely related to the question of computing generators for the multigraded regularity of $M$, and finding an efficient algorithm for that computation remains an interesting question.)

Next, we compute a finite part of the corner complex of $M$ at $c$.  The corner complex is an exact complex of the form
\[
\operatorname{Tail}_c(M) \to bgg(M_{\geq c}).
\]
Computing $\operatorname{Tail}_c(M)$ is now straightforward: since $bgg(M_{\geq c})$ is acyclic, we can simply take the minimal free resolution of first map of the total complex of $bgg(M_{\geq c})$.  For instance, if $t=2$ then we compute the corner complex by resolving the kernel of the map:
\[
M_c \otimes E(c) \to 
\begin{matrix}
M_{c+(1,0)} \otimes E(c+(1,0))\\
\bigoplus
\\
M_{c+(0,1)} \otimes E(c+(0,1)).
\end{matrix}
\]
The minimal free resolution of this map is the complex denoted $\operatorname{Tail}_c(M)$ in~\cite[\S1]{EES}.  A key fact from ~\cite[\S1]{EES} is that $\operatorname{Tail}_c(M)$ equals the subquotient complex of $\bT(\sF)$ obtained by restricting to degrees $\leq c - (1,1,\dots,1)$. By increasing $c$ and/or by computing enough terms of the free resolution $\operatorname{Tail}_c(M)$, we can thus compute any portion of $\bT(\sF)$.

%We can thus compute tate(M,high,low) as a subquotient complex of an appropriate corner complex.  Increasing $c$ if necessary, we can assume $c-(1,1,\dots,1)$ is at least as large as $high$.  And then we simply computing the minimal free resolution of \eqref{} for enough terms to ensure that we incorporate all potential summands generated in degree $low$.  

Moreover, by also computing enough terms of the injective resolution $bgg(M_{\geq c})$, we can also compute any portion of a corner complex.   
%though one should be a bit careful here.  
%However, one should be a bit careful when working with this construction.  
%The corner complex itself will many terms outside of the There are some odd features of corner complexes to be aware of.  
% long as $c-(1,1,\dots,1)$ degrees $high$ and $low$, we can ensure that this swath contains all summands generated in degre
%
% the Betti numbers of $F$ 
%which is
%$$ 
...
%
%
%All of our applications were based on our ability to compute a given finite part of the Tate resolution of a sheaf $\sF$ on $\PP$.  That computation is 
...
%
%%
%%Resolution at c yields a corner complex, which is correct
%%in all degrees $c-(1,1,...,1)$.
%%
%
%We then define $N$ as the kernel of the first map of the total complex of $bgg(M_{\geq c})$, which is
%$$ 
%M_c \otimes E(c) \to \bigoplus_{d=c+e_k}M_{d} \otimes E(d) 
%$$
%where $e_k \in \ZZ^t$ runs through the set of unit basis vectors.  Since $bgg(M_{\geq c})$ is an injective resolution of $N$, Theorem~\ref{Reciprocity} implies that $bgg(M_{\ge c})$ is an injective resolution of $N$.
...
\fix{What are low and high in this example?  Is this last sentence correct?}
\end{example}
