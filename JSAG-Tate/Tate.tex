...
****. The case of products of projective spaces is treated in ****. For a product of two or more projective
...
\begin{itemize}
 \item If $V,M_1, M_2$ are finite-dimensional vector spaces over a field $k$, then there is a one-to-one correspondence between homomorphisms $V\otimes_kM_1\to M_2$ and homomorphisms
$ M_1 \to M_2 \otimes V^*$.
\item Consider the polynomial ring $S = \Sym_kV$, with $V$ in degree 1, and the exterior algebra $E = \wedge_kV^*$, with $V^*$ in degree $-1$.
If $M = \oplus_{i\geq i_0} M_i$ is a finitely generated graded module over $S$, then the sequence of maps 
$$
V\otimes M_i \to M_{i+1}
$$
...
\begin{theorem}[Reciprocity \cite{EES}] With notation as above:
\begin{enumerate}
...
define bgg for one proj space. 

Where the cohomology appears.

****

Discovered/Used by  BGG 
